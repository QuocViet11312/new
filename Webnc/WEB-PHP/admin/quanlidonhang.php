<?php
    ...
    if ( ! empty($_SESSION['locsanpham2'])) {
        ...
        extract($_SESSION['locsanpham2']);//tự động lấy ra biến id và hoten
    }
    ...
    if ( ! empty($where)) {
        $result        = mysqli_query($con,
            "SELECT * FROM `oder` where (".$where.") ORDER BY `id` ASC  LIMIT "
            .$item_per_page." OFFSET ".$offset);
    }else{
        $result        = mysqli_query($con,
            "SELECT * FROM `oder` ORDER BY `id` ASC  LIMIT "
            .$item_per_page." OFFSET ".$offset);
    }
    ...
    <div class="main-content flex-1 bg-gray-100 mt-12 md:mt-2 pb-24 md:pb-5">
        ...
        <div class="bg-gray-800 pt-3">
            <div class="rounded-tl-3xl bg-gradient-to-r from-blue-900 to-gray-800 p-4 shadow text-2xl text-white">
                <h3 class="font-bold pl-2">Danh sách đơn hàng</h3>
            </div>
        </div>
        ...
        <div class="flex flex-wrap">
            ...
            <div class="flex w-full mr-8 justify-end">
                <div>Có tất cả <strong><?=$totalRecords?></strong> đơn hàng trên <strong><?=$totalPages?></strong> trang</div>
            </div>
            <div class="w-full p-6">
                <table id="table_customers">
                    ...
                    <tr class="border-2">
                        ...
                        <th class="w-1/12 ">Sản phẩm tháng</th>
                        <th class="w-2/12 ">Điện thoại</th>
                        <th class="w-2/12 ">Ngày tạo</th>
                        <th class="w-1/12 ">In đơn</th>
                        <th class="w-1/12 ">Hoàn tất</th>
                    </tr>
                    </thead>
                    <tbody>
                    <?php
                        while ($row = mysqli_fetch_array($result)) {
                            ...
                            <tr>
                                ...
                                <td class="text-center"><?= date('d/m/Y H:i',
                                        $row['ngaytao']) ?></td>
                                <td class="text-center doimau"> <a href="inhoadon.php?id=<?=$row['id']?>" target="_blank">In</a>
                                </td>
                                <td class="text-center doimau"><a
                                            href="./delete_qldonhang.php?id=<?= $row['id'] ?>">Duyệt</a>
                                </td>
                            </tr>
                            <?php
                        } ?>
